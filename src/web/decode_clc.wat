(module
  ...
  (func (export "decode_clc")
    ...
    (param $m_meta_ptr i32)
    (param $m_hi_x_ptr i32)
    (param $m_lo_x_ptr i32)
    (param $m_hi_y_ptr i32)
    (param $m_lo_y_ptr i32)
    (param $m_hi_i_ptr i32)
    (param $m_lo_i_ptr i32)
    (param $m_out_pos_ptr i32)
    (param $m_out_col_ptr i32)
    (param $m_out_ebo_ptr i32)
    (param $m_palette_ptr i32)
    ...
    (loop $poly_loop
      (block $poly_break
        ...
        ;; Read meta
        (local.set $code (i32.load16_u (i32.add (local.get $m_meta_ptr) (local.get $meta_ptr))))
        (local.set $v_count (i32.load16_u (i32.add (local.get $m_meta_ptr) (i32.add (local.get $meta_ptr) (i32.const 2)))))
        (local.set $t_count (i32.load16_u (i32.add (local.get $m_meta_ptr) (i32.add (local.get $meta_ptr) (i32.const 4)))))
        (local.set $meta_ptr (i32.add (local.get $meta_ptr) (i32.const 6)))

        ;; Lookup code index
        (local.set $code_idx (i32.load8_u (i32.add (local.get $m_palette_ptr) (local.get $code))))
            ...
            (if (i32.le_u (i32.add (local.get $k) (i32.const 8)) (local.get $v_count))
              (then
                ...
                (local.set $v_simd (v128.or 
                  (i16x8.shl (v128.load8x8_u (i32.add (local.get $m_hi_x_ptr) (i32.add (local.get $v_ptr) (local.get $k)))) (i32.const 8))
                  (v128.load8x8_u (i32.add (local.get $m_lo_x_ptr) (i32.add (local.get $v_ptr) (local.get $k))))
                ))
                ...
                ;; Y Coordinate
                (local.set $v_simd (v128.or 
                  (i16x8.shl (v128.load8x8_u (i32.add (local.get $m_hi_y_ptr) (i32.add (local.get $v_ptr) (local.get $k)))) (i32.const 8))
                  (v128.load8x8_u (i32.add (local.get $m_lo_y_ptr) (i32.add (local.get $v_ptr) (local.get $k))))
                ))
                ...
                (v128.store (i32.add (local.get $m_out_pos_ptr) (i32.shl (i32.add (local.get $out_v_ptr) (local.get $k)) (i32.const 2)))
                  (i8x16.shuffle 0 1 16 17 2 3 18 19 4 5 20 21 6 7 22 23 (local.get $vx_simd) (local.get $vy_simd)))
                (v128.store (i32.add (local.get $m_out_pos_ptr) (i32.add (i32.shl (i32.add (local.get $out_v_ptr) (local.get $k)) (i32.const 2)) (i32.const 16)))
              ...
              (else
                ;; Scalar fallback
                (local.set $zx (i32.or 
                  (i32.shl (i32.load8_u (i32.add (local.get $m_hi_x_ptr) (i32.add (local.get $v_ptr) (local.get $k)))) (i32.const 8))
                  (i32.load8_u (i32.add (local.get $m_lo_x_ptr) (i32.add (local.get $v_ptr) (local.get $k))))
                ))
                ...
                (local.set $zy (i32.or 
                  (i32.shl (i32.load8_u (i32.add (local.get $m_hi_y_ptr) (i32.add (local.get $v_ptr) (local.get $k)))) (i32.const 8))
                  (i32.load8_u (i32.add (local.get $m_lo_y_ptr) (i32.add (local.get $v_ptr) (local.get $k))))
                ))
                ...
                (i32.store16 (i32.add (local.get $m_out_pos_ptr) (i32.shl (i32.add (local.get $out_v_ptr) (local.get $k)) (i32.const 2))) (local.get $prev_x))
                (i32.store16 (i32.add (local.get $m_out_pos_ptr) (i32.add (i32.shl (i32.add (local.get $out_v_ptr) (local.get $k)) (i32.const 2)) (i32.const 2))) (local.get $prev_y))
                (i32.store8 (i32.add (local.get $m_out_col_ptr) (i32.add (local.get $out_v_ptr) (local.get $k))) (local.get $code_idx))
        ...
        (loop $i_loop
          (block $i_break
            ...
            (if (i32.le_u (i32.add (local.get $k) (i32.const 8)) (local.get $i_count))
              (then
                (local.set $v_simd (v128.or 
                  (i16x8.shl (v128.load8x8_u (i32.add (local.get $m_hi_i_ptr) (i32.add (local.get $i_ptr) (local.get $k)))) (i32.const 8))
                  (v128.load8x8_u (i32.add (local.get $m_lo_i_ptr) (i32.add (local.get $i_ptr) (local.get $k))))
                ))
                ...
                ;; Lower 4 elements
                (v128.store (i32.add (local.get $m_out_ebo_ptr) (i32.shl (i32.add (local.get $out_i_ptr) (local.get $k)) (i32.const 2)))
                  (i32x4.add (local.get $v_base) (i32x4.extend_low_i16x8_u (local.get $vi_simd))))
                ;; Upper 4 elements
                (v128.store (i32.add (local.get $m_out_ebo_ptr) (i32.add (i32.shl (i32.add (local.get $out_i_ptr) (local.get $k)) (i32.const 2)) (i32.const 16)))
              ...
              (else
                (local.set $zi (i32.or 
                  (i32.shl (i32.load8_u (i32.add (local.get $m_hi_i_ptr) (i32.add (local.get $i_ptr) (local.get $k)))) (i32.const 8))
                  (i32.load8_u (i32.add (local.get $m_lo_i_ptr) (i32.add (local.get $i_ptr) (local.get $k))))
                ))
                ...
                (i32.store (i32.add (local.get $m_out_ebo_ptr) (i32.shl (i32.add (local.get $out_i_ptr) (local.get $k)) (i32.const 2))) (i32.add (local.get $base_v) (local.get $prev_idx)))
                (local.set $k (i32.add (local.get $k) (i32.const 1)))
              )
            )
            (br $i_loop)
  ...
  (func (export "decode_water")
    ...
    (param $m_meta_ptr i32)
    (param $m_hi_x_ptr i32)
    (param $m_mid_x_ptr i32)
    (param $m_lo_x_ptr i32)
    (param $m_hi_y_ptr i32)
    (param $m_mid_y_ptr i32)
    (param $m_lo_y_ptr i32)
    (param $m_hi_i_ptr i32)
    (param $m_lo_i_ptr i32)
    (param $m_out_wpos_ptr i32)
    (param $m_out_col_ptr i32)
    (param $m_out_ebo_ptr i32)
    (param $m_palette_ptr i32)
      ...
      (block $poly_break
        ...
        (loop $v_loop
          (block $v_break
            ...
            (local.set $zx (i32.or (i32.load8_u (i32.add (local.get $m_lo_x_ptr) (local.get $idx))) 
              (i32.or (i32.shl (i32.load8_u (i32.add (local.get $m_mid_x_ptr) (local.get $idx))) (i32.const 8))
                      (i32.shl (i32.load8_u (i32.add (local.get $m_hi_x_ptr) (local.get $idx))) (i32.const 16)))))
            (local.set $prev_x (i32.and (i32.add (local.get $prev_x) (call $zigzag_i32 (local.get $zx))) (global.get $mask24)))

            (local.set $zy (i32.or (i32.load8_u (i32.add (local.get $m_lo_y_ptr) (local.get $idx))) 
              (i32.or (i32.shl (i32.load8_u (i32.add (local.get $m_mid_y_ptr) (local.get $idx))) (i32.const 8))
                      (i32.shl (i32.load8_u (i32.add (local.get $m_hi_y_ptr) (local.get $idx))) (i32.const 16)))))
            (local.set $prev_y (i32.and (i32.add (local.get $prev_y) (call $zigzag_i32 (local.get $zy))) (global.get $mask24)))

            (local.set $v_out (i32.add (local.get $m_out_wpos_ptr) (i32.shl (i32.add (local.get $out_v_ptr) (local.get $k)) (i32.const 3))))
            (i32.store (local.get $v_out) (local.get $prev_x))
            (i32.store (i32.add (local.get $v_out) (i32.const 4)) (local.get $prev_y))
            (i32.store8 (i32.add (local.get $m_out_col_ptr) (i32.add (local.get $out_v_ptr) (local.get $k))) (local.get $code_idx))
