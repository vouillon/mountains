...
module Encoder = struct
  ...
  let encode_vertices t (verts : float array) num_verts (min_x, min_y)
      (scale_x, scale_y) =
    let last_qx = ref 0 in
    let last_qy = ref 0 in
    let count = ref 0 in

    (* Reordering could happen here, but we assume input is already reasonably ordered 
       or we accept the input order. For better compression, we would reorder based on usage,
       but usually triangulation preserves some locality. *)
    for i = 0 to num_verts - 1 do
      ...
      (* Delta *)
      let dx = qx - !last_qx in
      let dy = qy - !last_qy in

      let zx = zigzag_encode dx in
      let zy = zigzag_encode dy in
  ...
  let encode_indices t indices =
    let len = Array.length indices in
    write_u16 t.indices len;
    Array.iter (fun idx -> write_u16 t.indices idx) indices;
    len
end
...
let process_tile db_path output_dir tile_name =
  ...
  (* Prepare Spatial Query (LAEA Bounds) *)
  (* We need to project the WGS84 bbox corners to LAEA to query the R-Tree 
     Since LAEA is not aligned with Lat/Lon, we project a dense envelop or just the 4 corners + midpoints 
     to be safe. For small 1-degree tiles, 4 corners is usually "okay" but to be strict we take min/max of corners. *)
  let laea_pts =
    [
  ...
  (* Output buffer for feature headers: Code(1) + NumTriangles(2) + NumVerts(2) + ... *)
  (* Let's stick to the requested simple binary structure *)
  ...
  let write_float f =
    ...
    output_byte out_ch (Int32.to_int (Int32.shift_right bits 0) land 0xFF);
    output_byte out_ch (Int32.to_int (Int32.shift_right bits 8) land 0xFF);
    output_byte out_ch (Int32.to_int (Int32.shift_right bits 16) land 0xFF);
    output_byte out_ch (Int32.to_int (Int32.shift_right bits 24) land 0xFF)
  in
  (* However the USER format said: min_lon, min_lat float32 *)
  (* I should probably write doubles or floats. Let's write them as 32-bit floats. *)
  write_float min_lon;
  write_float min_lat;
  write_float scale_x;
  write_float scale_y;
  ...
  let rec process_rows () =
    ...
    | Sqlite3.Rc.ROW ->
        ...
        | Sqlite3.Data.BLOB b ->
            let wkb_opt =
              (* Skip GPKG Header manually if Wkb_decode doesn't handle fully in string mode 
                 Actually Wkb_decode.decode_wkb handles it now. *)
              Some b
            in
                      ...
                      let float_polys =
                        Array.map
                          (fun rings ->
                            ...
                            (* We need to pack this into the (verts, polygon) structs expected by Clipper *)
                            (* This is a bit inefficient (list -> array -> array), but fine for offline tool *)
                            (Array.of_list outer_pts, raw_holes))
                          polys
                      in

                      (* 2. Flatten for Clipping *)
                      (* Current Polygon_clipping expects (verts, poly) *)
                      Array.iter
                        (fun (outer_verts, hole_arrays) ->
                          (* Construct full flat array for this polygon *)
                          let total_len =
                          ...
                          | Some (clipped_verts, clipped_poly) ->
                              ...
                              if Array.length tris > 0 then (
                                (* 5. Reorder & Encode *)
                                (* We have (clipped_verts) and (tris) which are indices into clipped_verts *)
                                (* We need to emit only used vertices, ideally in order of appearance in tris *)
                                ...
                                for i = 0 to Array.length tris - 1 do
                                  ...
                                  then (
                                    ...
                                    Printf.printf
                                      "Error: old_idx %d out of bounds (len %d)\n\
                                       %!"
                                      old_idx (Array.length map)
                                done;
                                ...
                                (* We write offsets/sizes to main stream? Or chunked? *)
                                (* User format: code, num_triangles, vertices, indices *)
                                (* Since we split bytes, we must write them in blocks per feature or all at end? *)
                                (* "Per entry: ... vertices: ... indices: ..." implies interleaved blocks *)
                                let tri_count_val = i_count / 3 in
                                (* Write num_triangles (u16) *)
                                ...
                                (* Write counts? Decoder needs to know how many vertices to read? *)
                                (* "vertices: float32[]" in user spec means implied count? *)
                                (* Actually improved User Spec: "num_triangles: uint16". Knowing num triangles (T), indices count is 3*T. *)
                                (* Vertices count (V). We usually need to store V explicitly OR verify from indices. *)
                                (* Let's add num_vertices (u16) to the entry header for safety. *)
